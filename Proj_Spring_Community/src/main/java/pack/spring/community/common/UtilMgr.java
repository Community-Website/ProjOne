...
public class UtilMgr {
	...
	public static String replace(String str, String pattern, String replace) {
		...
		int s = 0, e = 0;
		StringBuffer result = new StringBuffer();   // StringBuilder, StringBuffer <=> String
		// String a = "가나다";
		// a.indexOf("나");   1     |    a.reverse( ); "다나가"  (X)
		// syso(a); => "가나다"    |    syso(a);    "다나가"
		
		while ((e = str.indexOf("<", 0)) >= 0) {
			      // 지역변수 e의 값이 0보다 크면 true, 즉 1이상이면 true
			
			      result.append(str.substring(s, e));
			      result.append(replace);
			      s = e + pattern.length();   // s++
